...
BEGIN    
    ...
    SELECT stok, stok_gudang 
		INTO var_stok_etalase, var_stok_gudang
    FROM m_produk 
	WHERE produk_id = var_produk_id;
    ...
    UPDATE m_produk 
	SET stok = var_stok_etalase, stok_gudang = var_stok_gudang 
	WHERE produk_id = var_produk_id;
...
BEGIN		   
    ...
    SELECT stok, stok_gudang, harga_beli 
		INTO var_stok_sekarang, var_stok_gudang_sekarang, var_hpp_awal
    FROM m_produk 
	WHERE produk_id = var_produk_id;
...
BEGIN
	...
	FROM t_beli_produk 
	WHERE tanggal_tempo IS NOT NULL 
	...
	UPDATE m_supplier 
		SET total_hutang = var_total_hutang_produk, 
			total_pembayaran_hutang = var_total_pelunasan_produk 
	...
	IF TG_OP = 'UPDATE' THEN
		...
		IF var_supplier_id <> var_supplier_id_old THEN
			...
			FROM t_beli_produk
			WHERE tanggal_tempo IS NOT NULL 
				AND supplier_id = var_supplier_id_old;
			...
			UPDATE t_pembayaran_hutang_produk 
				SET supplier_id = var_supplier_id
			WHERE pembayaran_hutang_produk_id IN 
				(SELECT pembayaran_hutang_produk_id 
				FROM t_item_pembayaran_hutang_produk 
				WHERE beli_produk_id = NEW.beli_produk_id);
		END IF;
...
CREATE TABLE IF NOT EXISTS customergroup (
	customergroupid int NOT NULL GENERATED BY DEFAULT AS IDENTITY,
	name varchar(30) NOT NULL,
	description varchar(80) NULL,
	seqno int,
	isactive bool DEFAULT false,
	CONSTRAINT customergroup_pk PRIMARY KEY (customergroupid)
);

CREATE OR REPLACE FUNCTION f_customergroup_seqno()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
	NEW.seqno := NEW.customergrupid * 10;
	RETURN NEW;
END;
$function$
;
COMMENT ON FUNCTION f_customergroup_seqno()
    IS 'Set seqno to customergroupid * 10 as default value on insert.';

CREATE TRIGGER customergroup_seqno
    AFTER INSERT
    ON customergroup
    FOR EACH ROW
        EXECUTE PROCEDURE f_customergroup_seqno();

COMMENT ON TRIGGER customergroup_seqno ON customergroup
    IS 'Set seqno to customergroupid * 10 as default value on insert.';
...
CREATE TABLE productbundle (
	productbundleid t_guid NOT NULL,
	produk_id t_guid NOT NULL,
    subproductid t_guid NOT NULL,
	qty int4 NOT NULL DEFAULT 1,
    seqno int2 NULL DEFAULT 10,
	isactive bool NULL DEFAULT false,
	CONSTRAINT productbundle_pk PRIMARY KEY (productbundleid, produk_id)
);

-- productbundle foreign keys
ALTER TABLE productbundle ADD CONSTRAINT productbundle_fk FOREIGN KEY (subproductid) REFERENCES m_produk(produk_id);

-- ----------------------------------------------------------------------------
-- PAYMENT TERM
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS paymentterm (
  paymenttermid INTEGER GENERATED BY DEFAULT AS IDENTITY (INCREMENT 10 START 10),
  name CHARACTER VARYING(30) NOT NULL,
  isduefixed BOOLEAN NOT NULL DEFAULT FALSE,
  netdays INTEGER,
  dayofmonth INTEGER,
  ...
  updatedby t_guid,
  CONSTRAINT paymentterm_pk PRIMARY KEY (paymenttermid),
  CONSTRAINT paymentterm_name_uq UNIQUE (name)
);
