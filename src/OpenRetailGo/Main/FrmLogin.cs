...
namespace OpenRetailGo.Main
{
    ...
    {
        private ILog _log;
        private string _appConfigFile = string.Format("{0}\\OpenRetailGo.exe.config", Utils.GetAppPath());
        ...
        {
            if(AppConfigHelper.GetValue("server", _appConfigFile) != txtServer.Text.Trim())
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            using (new StCursor(Cursors.WaitCursor, new TimeSpan(0, 0, 0, 0)))
            {
                ...
                if (!dbConn.IsOpenConnection())
                {
                    var msg = "Maaf koneksi ke database gagal.\n";

                    if (!String.IsNullOrEmpty(((DbConnectionHelper)dbConn).DbError))
                        msg += ((DbConnectionHelper)dbConn).DbError;
                    else
                        msg += "Disarankan untuk menginstall OpenRetail di 'Drive D'.\n" +
            ...
            using (new StCursor(Cursors.WaitCursor, new TimeSpan(0, 0, 0, 0)))
            {
                ...
                if (isLogin)
                {
                    // TODO: upgrade db dibuat seperti opsi di utiliti
